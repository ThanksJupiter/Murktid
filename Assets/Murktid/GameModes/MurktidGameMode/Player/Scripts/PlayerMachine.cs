...
namespace Murktid {
    public class PlayerMachine : StateMachine<PlayerContext>, ICharacterController {
        public PlayerMachine(PlayerReference playerReference, ApplicationData applicationData) {
            context = playerReference.context;
        }

        public void Initialize() {
            context.motor.CharacterController = this;
            context.camera.PlanarDirection = context.camera.transform.forward;
            context.movement.activeMoveSpeed = context.settings.defaultMoveSpeed;
        ...
        public void SetInput() {
            ...
            Vector3 cameraPlanarDirection = Vector3.ProjectOnPlane(context.camera.transform.rotation * Vector3.forward, context.motor.CharacterUp).normalized;

            if(cameraPlanarDirection.sqrMagnitude == 0f) {
                cameraPlanarDirection = Vector3.ProjectOnPlane(context.camera.transform.rotation * Vector3.up, context.motor.CharacterUp).normalized;
            }
        ...
        private void UpdateCameraPositionAndRotation() {
            ...
            Quaternion planarRotation =
                Quaternion.LookRotation(context.camera.PlanarDirection, context.motor.CharacterUp);
            Quaternion targetRotation = Quaternion.Slerp(context.camera.transform.rotation, planarRotation * verticalRotation,
        ...
        public void UpdateVelocity(ref Vector3 currentVelocity, float deltaTime) {
            ...
            float stableMoveSpeed = context.movement.activeMoveSpeed; // context.statusEffectSystem.GetStatusEffectedMovementSpeed(context.movement.activeMoveSpeed);
